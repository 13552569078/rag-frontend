...
import { apiKnowChatUrl, apiKnowLLmChatUrl } from "@/api/document";
...
const KnowChat: React.FC = () => {
  ...
  const send = (q: string) => {
    ...
    // 处理URL
    const url = modelType === DEFAULT_MODEL_TYPE ? apiKnowChatUrl : apiKnowLLmChatUrl;
    ...
    <div className="w-full h-full grid grid-cols-5">
      ...
      <div className="flex flex-col overflow-y-hidden col-span-2">
        <div className="py-[20px] px-[30px] bg-[#F9FAFE]">
          <Back type={2} bread={`${paramsQuery?.kbname}知识空间`} />
        </div>
        {/* 溯源为空 亦或 溯源回显 */}
